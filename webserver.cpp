...
{
    //网络编程基础步骤
    m_listenfd = socket(PF_INET, SOCK_STREAM, 0); //IPv4网络协议，面向流服务或流套接字
    assert(m_listenfd >= 0);
...
void WebServer::timer(int connfd, struct sockaddr_in client_address)
{
    ...
    //创建定时器并加入对应的时间轮槽内
    tw_timer *timer = utils.m_timer_lst.add_timer(3 * TIMESLOT);
    //设置定时器对应的连接资源
...
//若有数据传输，说明该连接处于活动，则将定时器往后延迟3个单位
//并对新的定时器在链表上的位置进行调整
void WebServer::adjust_timer(tw_timer *timer)
{
    //刷新超时时间
    timer->timeout = 3 * TIMESLOT;
...
void WebServer::eventLoop()
{
    ...
    while (!stop_server)
    {
        ...
        int number = epoll_wait(m_epollfd, events, MAX_EVENT_NUMBER, -1); //阻塞，等待这个函数执行完毕
